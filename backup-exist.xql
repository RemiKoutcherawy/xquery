...
declare function local:setPermissionsFromContent($col) as item()* {
...
(: Permissions on collection :)
  ...
  let $void := for $r in $content/exist:resource return
    ...
    let $void := xmldb:chmod-resource($col, $name, util:base-to-integer($mode, 8))
    return ()
...
declare function local:entry-data($path as xs:anyURI, $type as xs:string, $data as item ()?, $param as item ()*) as item ()? {
  ...
  let $colParent := "/" || string-join(subsequence($toks, 1, count($toks) - $i - 1), "/")
  let $colName := $toks[count($toks) - $i]
  return
    if ( xmldb:collection-available($colPath) ) then ()
    else xmldb:create-collection($colParent, $colName)
  ...
  let $data :=
    ...
    mode="{sm:mode-to-octal($p/@mode)}" created="{current-dateTime()}" version="1">
      ...
      (: Resources at this level :)
        ...
        return
          <subcollection name="{ $c }" filename="{ $c }"/>
      }
    </collection>
...
declare function local:remove__contents__($colName as xs:string) as item()* {
  if (doc-available($colName || "/__contents__.xml"))
  then xmldb:remove($colName, "__contents__.xml")
  else ()
};

(: Download. "action" = "Download" :)
let $done := if (request:get-parameter("action", ()) = "Download") then
  let $ouv := request:get-parameter("collection", "")
  let $rootCol := "/db/" || $ouv || "/"
  let $name := $ouv || ".zip"

  (: Add __contents__.xml, zip, remove __contents__.xml :)
  let $void := local:scan-collections($rootCol, local:add__contents__ #1)
  let $zip := compression:zip(xs:anyURI($rootCol), true())
  let $void := local:scan-collections($rootCol, local:remove__contents__ #1)

  (: Return binary to trigger navigator 'download as' :)
  return
    (
...
(: $zipFile gets data send by <input type="file" name="zipFile" :)
  ...
  let $unzip :=
    try {
      ...
      let $void := local:scan-collections($rootCol, local:setPermissionsFromContent #1)
      let $void := local:scan-collections($rootCol, local:remove__contents__ #1)
